...
class CarpetaController extends BaseController
{
    ...
    public function create()
    {
        if ($this->request->getMethod() == 'POST') {
            ...
            if (empty($nombre)) {
                $res = ['type' => 'warning', 'msg' => 'El nombre el requerido'];
            } else {
    ...
    public function delete($id)
    {
        if ($this->request->getMethod() == "DELETE") {
            $this->eliminarArchivosFisicos($id);
    ...
    public function eliminarArchivosFisicos($id_carpeta)
    {
        //ELIMINAR LA CARPETA ACTUAL
        $archivos = $this->archivoModel->where('carpeta_id', $id_carpeta)->findAll();
        ...
        //OBTENER SUBCARPETAS
        $subcarpetas = $this->carpetaModel
        ->where('carpetapadre_id', $id_carpeta)->findAll();
        foreach ($subcarpetas as $subcarpeta) {
            $this->eliminarArchivosFisicos($subcarpeta['carpeta_id']);
        }
    }

    public function show($id)
    {
        //COMPROBAR SI LA CARPETA ME PERTENECE
        $existe = $this->carpetaModel->find($id);
        if ($existe == null || $existe['user_id'] != $this->session->user_id) {
            return redirect()->to('carpetas')->with('respuesta', [
                'type' => 'danger',
                'msg' => 'No tienes permisos'
            ]);
        }
        $carpetas = $this->carpetaModel
            ->where('carpetapadre_id', $id)->orderBy('carpeta_id', 'desc')
            ->paginate(12);
